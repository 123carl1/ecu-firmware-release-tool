...
class As5prOtaStateMachine:
    def __init__(self, transport: Any, progress: Callable[[str], None] | None = None,
                 sleep_func: Callable[[float], None] = sleep) -> None:
        self._transport = transport
        self._progress = progress
        self._config = get_project_config(ProjectCode.AS5PR)
        self._app_erased = False
        self._sleep = sleep_func

    def run(
        self,
        package: VerifiedReleasePackage,
        ...
        try:
            ...
            return self._verify_after_reset(package, cancel_token)
        except OperationCancelled:
            status = OtaResultStatus.ECU_IN_BOOT if self._app_erased else OtaResultStatus.CANCELLED_SAFE
            return OtaResult(status, package.release_set_id, "取消已在协议安全点生效")
        except Exception as exc:  # 状态机边界必须把异常映射为稳定结果。
            status = OtaResultStatus.FAILED_UNKNOWN if self._app_erased else OtaResultStatus.CANCELLED_SAFE
            return OtaResult(status, package.release_set_id, str(exc))

    def _package_matches(self, package: VerifiedReleasePackage) -> bool:
        cfg = self._config
    ...
    def _download(self, address: int, data: bytes, token: CancellationToken | None) -> None:
        cfg = self._config
        ...
        if len(response) < 3 or int.from_bytes(response[2:], "big") < cfg.timing.max_transfer_payload + 2:
            raise ValueError("ECU 声明的下载块长度不足")
        crc = AS5PR_CRC32_INIT
        sequence = 1
        for chunk in split_transfer_chunks(data, cfg.timing.max_transfer_payload):
